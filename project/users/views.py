...
#helper function to help sending emails to multiple users
...
# adding a helper function to actually execute the sending of email in a separate thread:
...
#  code for processing the unique link send to a user when they register
...
# helper function to send the email with the link to allow the user to reset their password
...
# the actual route that uses this helper function for password reset
...
# The route for handling the password reset link is quite complicated,
# as it requires first checking the token’s validity and then processing
#the new password provided by the user through the form.
